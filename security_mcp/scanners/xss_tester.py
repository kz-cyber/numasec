...
logger = logging.getLogger("security_mcp.scanners.xss_tester")
...
class PythonXSSTester:
    ...
    _STORED_XSS_PAYLOADS: list[str] = [
        "<script>alert('SECMCP_STORED')</script>",
        "<img src=x onerror=alert('SECMCP_STORED')>",
        "<svg onload=alert('SECMCP_STORED')>",
    ]
            ...
            for payload in self._STORED_XSS_PAYLOADS:
                try:
                    ...
                    if "reviews" in write_path:
                        body.update({"author": "security_mcp@test.com"})
    ...
    _BROWSER_DOM_PAYLOADS: list[str] = [
        "<img src=x onerror=alert('SECMCP_XSS')>",
        "<iframe src=\"javascript:alert('SECMCP_XSS')\">",
        "<svg onload=alert('SECMCP_XSS')>",
        "<details open ontoggle=alert('SECMCP_XSS')>",
    ]
    ...
    async def _browser_probe_xss(page: Any, test_url: str) -> bool:
        ...
        def on_dialog(dialog: Any) -> None:
            ...
            if "SECMCP_XSS" in (dialog.message or ""):
                dialog_fired = True
