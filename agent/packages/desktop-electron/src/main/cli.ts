...
const CLI_INSTALL_DIR = ".numasec/bin"
const CLI_BINARY_NAME = "numasec"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `numasec-cli${suffix}`)
    : join(root, "../../resources", `numasec-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "numasec-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    NUMASEC_SERVER_USERNAME: "numasec",
    NUMASEC_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    NUMASEC_EXPERIMENTAL_ICON_DISCOVERY: "true",
    NUMASEC_EXPERIMENTAL_FILEWATCHER: "true",
    NUMASEC_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.numasec/bin/numasec"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://numasec.ai/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",
