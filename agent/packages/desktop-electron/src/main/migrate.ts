...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.numasec.desktop.dev",
  beta: "ai.numasec.desktop.beta",
  prod: "ai.numasec.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.numasec.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `numasec.settings.dat` is special: it maps to the `numasec.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"numasec.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // numasec.settings.dat → the electron settings store ("numasec.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "numasec.global.dat").
  const storeName = filename === "numasec.settings.dat" ? "numasec.settings" : filename
