...
app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Numasec Dev")
app.setPath("userData", join(app.getPath("appData"), app.isPackaged ? APP_IDS[CHANNEL] : "ai.numasec.desktop.dev"))
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("numasec://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    // migrate()
    app.setAsDefaultProtocolClient("numasec")
    setDockIcon()
    setupAutoUpdater()
    syncCli()
    await initialize()
...
async function getSidecarPort() {
  const fromEnv = process.env.NUMASEC_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "numasec", "numasec.db"))
}
