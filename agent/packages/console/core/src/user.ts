  ...
  export const invite = fn(
    ...
    async ({ email, role, monthlyLimit }) => {
      ...
      try {
        ...
        const { InviteEmail } = await import("@numasec/console-mail/InviteEmail.jsx")
        await AWS.sendEmail({
          to: email,
          subject: `You've been invited to join the ${emailInfo.workspaceName} workspace on Numasec`,
          body: render(
            // @ts-ignore
            InviteEmail({
              inviter: emailInfo.inviterEmail,
              assetsUrl: `https://numasec.ai/email`,
