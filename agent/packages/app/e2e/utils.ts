...
import { base64Encode, checksum } from "@numasec/util/encode"
...
export function createSdk(directory?: string) {
  return createNumasecClient({ baseUrl: serverUrl, directory, throwOnError: true })
}
...
export function workspacePersistKey(directory: string, key: string) {
  ...
  return `numasec.workspace.${head}.${sum}.dat:workspace:${key}`
}
