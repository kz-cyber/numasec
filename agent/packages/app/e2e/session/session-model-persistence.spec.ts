...
async function probe(page: Page): Promise<Probe | null> {
  return page.evaluate(() => {
    ...
    return win.__numasec_e2e?.model?.current ?? null
  })
}
    ...
    .poll(
      () =>
        page.evaluate((key) => {
          ...
          return !!win.__numasec_e2e?.model?.controls?.[key]
        }, key),
      { timeout: 30_000 },
    )
    .toBe(true)
...
async function pickAgent(page: Page, value: string) {
  ...
  await page.evaluate((value) => {
    ...
    const fn = win.__numasec_e2e?.model?.controls?.setAgent
...
async function pickModel(page: Page, value: { providerID: string; modelID: string }) {
  ...
  await page.evaluate((value) => {
    ...
    const fn = win.__numasec_e2e?.model?.controls?.setModel
...
async function pickVariant(page: Page, value: string) {
  ...
  await page.evaluate((value) => {
    ...
    const fn = win.__numasec_e2e?.model?.controls?.setVariant
