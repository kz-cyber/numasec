...
async function todoDock(page: any, sessionID: string) {
  ...
  const write = async (driver: ComposerDriverState | undefined) => {
    await page.evaluate(
      (input: { event: string; sessionID: string; driver: ComposerDriverState | undefined }) => {
        const win = window as ComposerWindow
        const composer = win.__numasec_e2e?.composer
  ...
  const read = () =>
    page.evaluate((sessionID: string) => {
      const win = window as ComposerWindow
      return win.__numasec_e2e?.composer?.sessions?.[sessionID]?.probe ?? null
  ...
  await withDockSession(sdk, "e2e composer dock permission once", async (session) => {
    ...
    await withMockPermission(
      page,
      {
        ...
        sessionID: session.id,
        permission: "bash",
        patterns: ["/tmp/numasec-e2e-perm-once"],
  ...
  await withDockSession(sdk, "e2e composer dock permission reject", async (session) => {
    ...
    await withMockPermission(
      page,
      {
        ...
        sessionID: session.id,
        permission: "bash",
        patterns: ["/tmp/numasec-e2e-perm-reject"],
      },
      undefined,
      async (state) => {
        await page.goto(page.url())
  ...
  await withDockSession(sdk, "e2e composer dock permission always", async (session) => {
    ...
    await withMockPermission(
      page,
      {
        ...
        sessionID: session.id,
        permission: "bash",
        patterns: ["/tmp/numasec-e2e-perm-always"],
        metadata: { description: "Need permission for command" },
      },
      undefined,
      async (state) => {
    ...
    try {
      await withMockPermission(
        page,
        {
          ...
          sessionID: child.id,
          permission: "bash",
          patterns: ["/tmp/numasec-e2e-perm-child"],
