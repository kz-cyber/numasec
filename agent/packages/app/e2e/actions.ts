import { base64Decode, base64Encode } from "@numasec/util/encode"
...
export async function terminalConnects(page: Page, input?: { term?: Locator }) {
  ...
  return page.evaluate((id) => {
    return (window as E2EWindow).__numasec_e2e?.terminal?.terminals?.[id]?.connects ?? 0
  }, id)
}
...
export async function disconnectTerminal(page: Page, input?: { term?: Locator }) {
  ...
  await page.evaluate((id) => {
    ;(window as E2EWindow).__numasec_e2e?.terminal?.controls?.[id]?.disconnect?.()
  }, id)
}
...
async function terminalReady(page: Page, term?: Locator) {
  ...
  return page.evaluate((id) => {
    const state = (window as E2EWindow).__numasec_e2e?.terminal?.terminals?.[id]
...
async function terminalHas(page: Page, input: { term?: Locator; token: string }) {
  ...
  return page.evaluate(
    (input) => {
      const state = (window as E2EWindow).__numasec_e2e?.terminal?.terminals?.[input.id]
...
async function promptSlashActive(page: Page, id: string) {
  return page.evaluate((id) => {
    const state = (window as E2EWindow).__numasec_e2e?.prompt?.current
...
async function promptSlashSelects(page: Page) {
  return page.evaluate(() => {
    return (window as E2EWindow).__numasec_e2e?.prompt?.current?.selects ?? 0
  })
}
...
export async function seedProjects(page: Page, input: { directory: string; extra?: string[] }) {
  await page.addInitScript(
    (args: { directory: string; serverUrl: string; extra: string[] }) => {
      const key = "numasec.global.dat:server"
...
export async function createTestProject() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "numasec-e2e-project-"))
  const id = `e2e-${path.basename(root)}`

  await fs.writeFile(path.join(root, "README.md"), `# e2e\n\n${id}\n`)

  execSync("git init", { cwd: root, stdio: "ignore" })
  await fs.writeFile(path.join(root, ".git", "numasec"), id)
...
async function probeSession(page: Page) {
  return page
    .evaluate(() => {
      ...
      const current = win.__numasec_e2e?.model?.current
