...
test("changing language updates settings labels", async ({ page, gotoSession }) => {
  await page.addInitScript(() => {
    localStorage.setItem("numasec.global.dat:language", JSON.stringify({ locale: "en" }))
  })
...
test("legacy oc-1 theme migrates to oc-2", async ({ page, gotoSession }) => {
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return localStorage.getItem("numasec-theme-css-light")
      })
    })
    .toBeNull()
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return localStorage.getItem("numasec-theme-css-dark")
      })
    })
    .toBeNull()
})
