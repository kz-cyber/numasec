...
const numasecDir = path.join(repoDir, "packages", "numasec")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "numasec-e2e-"))
const keepSandbox = process.env.NUMASEC_E2E_KEEP_SANDBOX === "1"
...
const serverEnv = {
  ...
  NUMASEC_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  NUMASEC_E2E_PROJECT_DIR: repoDir,
  NUMASEC_E2E_SESSION_TITLE: "E2E Session",
  NUMASEC_E2E_MESSAGE: "Seeded for UI e2e",
  NUMASEC_E2E_MODEL: "numasec/gpt-5-nano",
  NUMASEC_CLIENT: "app",
  NUMASEC_STRICT_CONFIG_DEPS: "true",
...
const runnerEnv = {
  ...
  VITE_NUMASEC_SERVER_HOST: "127.0.0.1",
  VITE_NUMASEC_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../numasec/src/util/log")
    const install = await import("../../numasec/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../numasec/src/server/server")
    inst = await import("../../numasec/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`numasec server listening on http://127.0.0.1:${serverPort}`)
