...
export const modelEnabled = () => {
  ...
  return (window as ModelWindow).__numasec_e2e?.model?.enabled === true
}

const root = () => {
  if (!modelEnabled()) return
  return (window as ModelWindow).__numasec_e2e?.model
}

export const modelProbe = {
  bind(id: symbol, input: NonNullable<NonNullable<ModelWindow["__numasec_e2e"]>["model"]>["controls"]) {
    const state = root()
    if (!state) return
    active = id
