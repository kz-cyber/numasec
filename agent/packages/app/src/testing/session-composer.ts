...
export const composerEvent = "numasec:e2e:composer"
...
export const composerEnabled = () => {
  ...
  return (window as ComposerWindow).__numasec_e2e?.composer?.enabled === true
}

const root = () => {
  if (!composerEnabled()) return
  const state = (window as ComposerWindow).__numasec_e2e?.composer
  if (!state) return
  state.sessions ??= {}
