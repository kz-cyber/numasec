...
import { type Session } from "@numasec/sdk/v2/client"
import {
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("numasec://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("numasec://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("numasec://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("numasec://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("numasec://open-project")).toBeUndefined()
    expect(parseDeepLink("numasec://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "numasec://open-project?directory=/a",
      "numasec://other?directory=/b",
      "numasec://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("numasec://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("numasec://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("numasec://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("numasec://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "numasec://new-session?directory=/a",
      "numasec://open-project?directory=/b",
      "numasec://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NUMASEC__: {
        deepLinks: ["numasec://open-project?directory=/a"],
      },
    } as unknown as Window & { __NUMASEC__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["numasec://open-project?directory=/a"])
