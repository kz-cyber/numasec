...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "numasec.json")).href
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-numasec@2.4.3")).toBe("oh-my-numasec")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@numasec/plugin@2.0.0")).toBe("@numasec/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    ...
    expect(result[0]).toBe("file:///project/.numasec/plugin/oh-my-numasec.js")
  })
  ...
  test("local plugin directory overrides global numasec.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const numasecDir = path.join(projectDir, ".numasec")
