...
test("discovers skills from .numasec/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".numasec", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".numasec", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".numasec", "skill", "dir-skill")
...
test("discovers multiple skills from .numasec/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".numasec", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".numasec", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".numasec", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.numasec/skills/ directory", async () => {
  ...
  try {
    ...
    process.env.NUMASEC_TEST_HOME = originalHome
  }
})
...
`,
      ...
      await Bun.write(
        path.join(numasecSkillDir, "SKILL.md"),
        `---
name: numasec-skill
...
`,
      ...
      await Bun.write(
        path.join(numasecSkillsDir, "SKILL.md"),
        `---
name: numasec-skill
description: A skill in the .numasec/skills directory.
---
