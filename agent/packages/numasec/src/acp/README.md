...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for numasec.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal numasec sessions
...
numasec acp --cwd /path/to/project
```
...
```bash
NUMASEC_ENABLE_QUESTION_TOOL=1 numasec acp
```
...
- **Terminal Integration**: Full terminal support via numasec's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | numasec acp
```
...
ACP sessions map cleanly to numasec's internal session model:
