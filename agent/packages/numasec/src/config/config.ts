...
export namespace Config {
  ...
  export async function installDependencies(dir: string) {
    ...
    await BunProc.run(
      ...
      if (err instanceof Process.RunFailedError) {
        ...
        if (Flag.NUMASEC_STRICT_CONFIG_DEPS) {
  ...
  export async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@numasec/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@numasec/plugin", depVersion, dir)
   ...
   * getPluginName("oh-my-numasec@2.4.3") // "oh-my-numasec"
   ...
   * 2. Local numasec.json
   * 3. Global plugin/ directory
   * 4. Global numasec.json
   *
  ...
  export function deduplicatePlugins(plugins: string[]): string[] {
    ...
    // e.g., "oh-my-numasec", "@scope/pkg"
    const seenNames = new Set<string>()

    // uniqueSpecifiers: full plugin specifiers to return
    // e.g., "oh-my-numasec@2.4.3", "file:///path/to/plugin.js"
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@numasec/Config") {}
      ...
      Effect.gen(function* () {
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          if (Flag.NUMASEC_CONFIG) {
            result = mergeConfigConcatArrays(result, yield* loadFile(Flag.NUMASEC_CONFIG))
          ...
          if (process.env.NUMASEC_CONFIG_CONTENT) {
            result = mergeConfigConcatArrays(
              result,
              yield* loadConfig(process.env.NUMASEC_CONFIG_CONTENT, {
                dir: ctx.directory,
                source: "NUMASEC_CONFIG_CONTENT",
              }),
            )
