...
async function numasecFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.NUMASEC_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("numasec", Instance.directory, Instance.worktree)
  ...
  files.push(...ConfigPaths.fileInDirectory(input.managed, "numasec"))
