...
Replace runtime Bun shell template-tag usage in `packages/numasec/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/numasec/src` is removed except:
