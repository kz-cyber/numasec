...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("numasec-find")
  api.delete("numasec-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("numasec-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("numasec-find", new Highlight(...rest))
    return true
  }
