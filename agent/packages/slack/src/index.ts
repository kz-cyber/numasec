...
import { createnumasec, type ToolPart } from "@numasec/sdk"
...
console.log("🚀 Starting numasec server...")
const numasec = await createNumasec({
  port: 0,
})
console.log("✅ numasec server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await numasec.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new numasec session...")
    const { client, server } = numasec
    ...
    console.log("✅ Created numasec session:", createResult.data.id)
  ...
  console.log("📝 Sending to numasec:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 numasec response:", JSON.stringify(result, null, 2))
