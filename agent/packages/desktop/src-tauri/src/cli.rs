...
const CLI_INSTALL_DIR: &str = ".numasec/bin";
const CLI_BINARY_NAME: &str = "numasec";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("numasec-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "NUMASEC_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "NUMASEC_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("NUMASEC_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.numasec/bin/numasec\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://numasec.ai/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "NUMASEC_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "NUMASEC_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "NUMASEC_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "NUMASEC_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "NUMASEC_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "NUMASEC_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("NUMASEC_SERVER_USERNAME", "numasec".to_string()),
        ("NUMASEC_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn numasec");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("NUMASEC_CLIENT"), Some(&"desktop".to_string()));
    }
