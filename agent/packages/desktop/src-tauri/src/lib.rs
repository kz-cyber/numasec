...
async fn initialize(app: AppHandle) {
    ...
    let _ = ready_tx.send(ServerReadyData {
        url: url.clone(),
        username: Some("numasec".to_string()),
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %numasec_db_path().expect("failed to get db path").display(),
...
fn get_sidecar_port() -> u32 {
    option_env!("NUMASEC_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("NUMASEC_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = numasec_db_path() else {
        return true;
    };
...
fn numasec_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("numasec").join("numasec.db"))
}
