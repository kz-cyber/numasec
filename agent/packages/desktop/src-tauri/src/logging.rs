...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("numasec-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("numasec_lib=debug,numasec_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("numasec_lib=info,numasec_desktop=info,sidecar=info")
        }
    });
