...
const model = "numasec/gpt-5.3-codex"
...
async function typecheck() {
  ...
  try {
    await $`bun typecheck`.cwd("packages/numasec")
    return true
  } catch (err) {
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/numasec")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with numasec...`)
  ...
  const prompt = [
    ...
    "After resolving the conflicts, run `bun typecheck` in `packages/numasec`.",
  ...
  try {
    await $`numasec run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  numasec failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with numasec")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with numasec...")

  const done = lines(prs.filter((x) => applied.includes(x.number)))
  const prompt = [
    "The beta merge batch is complete.",
    `Merged PRs on HEAD:\n${done}`,
    "Run `bun typecheck` in `packages/numasec`.",
    "Run `./script/build.ts --single` in `packages/numasec`.",
