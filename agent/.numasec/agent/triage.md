...
**Only** add if it's specifically about the Desktop application or `numasec web` view. **Do not** add for terminal, TUI, or general numasec issues.
...
**Only** add if the issue mentions "zen" or "numasec zen" or "numasec black".

If the issue doesn't have "zen" or "numasec black" in it then don't add zen label

#### core

Use for core server issues in `packages/numasec/`, excluding `packages/numasec/src/cli/cmd/tui/`.
...
TUI (`packages/numasec/src/cli/cmd/tui/...`):
...
Core (`packages/numasec/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for numasec server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
